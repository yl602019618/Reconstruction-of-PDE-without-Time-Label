...
from NIOModules import NIOFP2D, NIOFP2D_FNO, NIOFP2D_FNO_attn, PermInvUNet_attn
...
class TrajectoryDataset2D(Dataset):
    def __init__(self, file_path):
        ...
        self.F = np.array(data["F"], dtype=np.float32)*1e12        # Shape: (M, Nx, Ny)
        ...
        self.F_mean = self.F.mean(axis=(0), keepdims=True)
        self.F_std = self.F.std(axis=(0), keepdims=True) + 1e-8
    
        
        # Standardize data
        self.trajectories = (self.trajectories - self.trajectories_mean) / self.trajectories_std
        self.F = (self.F - self.F_mean) / self.F_std
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        output_F = torch.tensor(self.F[idx], dtype=torch.float32).permute(1,2,0)   # Shape: (Nx, Ny,2)
...
# ------------------------ 数据加载 ------------------------
file_path = "/home/ubuntu/unlabelPDE_official/2d_force/dataset_2D_drift.npz"
batch_size = 4
...
nx = 80
ny = 80
model =  PermInvUNet_attn(in_ch=1, out_ch=2, base_ch=1, depth=5, input_size=(nx, ny))
...
for epoch in range(1, num_epochs + 1):
    ...
    if epoch % save_interval == 0:
        ...
        plt.yscale('log')
        plt.grid(True)
        plt.savefig(f"result_unet/loss_curve.png")
        plt.close()
        ...
        if test_loss < best_test_loss:
            ...
            best_ckpt_path = f"result_unet/model_checkpoint_best_{best_test_loss:.6f}.pt"
        ...
        # 反归一化
        F_std = torch.tensor(dataset.F_std, dtype=torch.float32, device=device).permute(0,2,3,1)
        true_F = test_outputs * F_std
        pred_F = predictions * F_std
        
        
        # 绘制预测结果图（仅展示潜力图）
        fig, axes = plt.subplots(2, 2, figsize=(12, 12))
        ax = axes[0, 0]
        im = ax.imshow(true_F[0,:,:,0].cpu().numpy(), cmap="viridis", origin="lower",vmax = 0)
        ax.set_title("True Potential")
        plt.colorbar(im, ax=ax)
        
        ax = axes[0, 1]
        im = ax.imshow(pred_F[0,:,:,0].detach().cpu().numpy(), cmap="viridis", origin="lower",vmax = 0)
        ax.set_title("Predicted Potential")
        plt.colorbar(im, ax=ax)
        
        ax = axes[1, 0]
        im = ax.imshow(true_F[0,:,:,1].cpu().numpy(), cmap="viridis", origin="lower",vmax = 0)
        ax.set_title("True Potential")
        plt.colorbar(im, ax=ax)
        
        ax = axes[1, 1]
        im = ax.imshow(pred_F[0,:,:,1].detach().cpu().numpy(), cmap="viridis", origin="lower",vmax = 0)
        ax.set_title("Predicted Potential")
        plt.colorbar(im, ax=ax)
        
        plt.suptitle(f"Test Predictions at Epoch {epoch}, Test Loss: {test_loss:.6f}")
        plt.savefig(f"result_unet/F_epoch_{epoch}.png")
        plt.close()




np.save("result_unet/train_losses.npy", np.array(train_losses))
np.save("result_unet/test_losses.npy", np.array(test_losses))
np.save("result_unet/test_losses_drift.npy", np.array(test_losses_drift))
np.save("result_unet/test_losses_diffusion.npy", np.array(test_losses_diffusion))