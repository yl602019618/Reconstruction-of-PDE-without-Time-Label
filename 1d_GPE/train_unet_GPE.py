...
file_path = "/home/ubuntu/unlabel_PDE_official/1dGPE/training_data_GPE.npy"
batch_size = 32
# Create dataset
...
model = PermInvUNet_attn1D_bag_GPE(in_ch=1, out_ch=2, base_ch=1, depth=4, input_size=128,device = device, width = width, modes = modes).to(device)
...
os.makedirs("results_GPE_unet", exist_ok=True)
...
for epoch in tqdm(range(1, num_epochs + 1)):
    ...
    if epoch % save_interval == 0:
        ...
        if test_loss < best_test_loss:
            ...
            best_ckpt_path = f"results_GPE_unet/model_checkpoint_best_{best_test_loss:.6f}.pt"
        ...
        plt.grid(True)
        plt.yscale('log')
        plt.savefig("results_GPE_unet/loss_curve.png")
        plt.close()
        # 可视化一个测试样本
        model.eval()
        ...
        plt.tight_layout()
        plt.savefig(f"results_GPE_unet/{epoch}_pred_vs_true.png")
        plt.close()


np.save("results_GPE_unet/train_losses.npy", np.array(train_losses))
np.save("results_GPE_unet/test_losses.npy", np.array(test_loss_curve))
