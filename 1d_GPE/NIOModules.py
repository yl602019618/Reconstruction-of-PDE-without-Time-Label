...
class NIOFP(nn.Module):
    def __init__(self,
        ...
        self.device = device
        self.fno_V = FNO1d(modes = modes,
                    width = width,
                    n_layers = self.fno_layers,
                    input_dim  = width,
                    output_dim = 1,
                    device = self.device)
        self.fno_gk = FNO1d(modes = modes,
                    width = width,
                    n_layers = self.fno_layers,
                    input_dim  = width,
                    output_dim = 2,
    ...
    def forward(self, x, grid):
        ...
        x_v = self.fno_V(x)
        x_gk = self.fno_gk(x)
        x = torch.cat((x_v,x_gk),dim = -1)
     
        return x

class NIOFP_schrodinger(nn.Module):
    def __init__(self,
                 input_dimensions_trunk,
                 n_hidden_layers,
                 neurons,
                 ...
                 output_dim,
                 device):
        super(NIOFP_schrodinger, self).__init__()
        output_dimensions = n_basis
        # self.fno_inputs = fno_input_dimension
        self.trunk = FFN(input_dimensions_trunk, 
                                    output_dimensions, 
...
class PermInvUNet_attn1D_bag_GPE(nn.Module):
    def __init__(self, in_ch=1, out_ch=2, base_ch=1, depth=4, input_size=61,device = None,width = None, modes = None):
        super().__init__()
        self.device = device 
        self.depth = depth
        self.width = width
        ...
        self.final_conv = nn.Conv1d(self.chs[0], self.width, 1)
        self.modes = modes
        self.fno_V = FNO1d(modes=self.modes,
                    width = self.width,
                    n_layers = 3,
                    input_dim  = self.width,
                    output_dim = 1,
