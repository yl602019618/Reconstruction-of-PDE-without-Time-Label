...
from NIOModules import NIOFP_schrodinger, NIOFP_FNO, PermInvUNet_attn1D_bag
import random
import numpy as np
import torch
...
model = PermInvUNet_attn1D_bag(in_ch=1, out_ch=2, base_ch=1, depth=4, input_size=128,device = device).to(device)
...
os.makedirs("results_schrodinger_unet", exist_ok=True)
...
for epoch in tqdm(range(1, num_epochs + 1)):
    ...
    for inputs, targets in train_loader:
        ...
        optimizer.zero_grad()
        predictions = model(inputs)  # 输出 shape: (batch, 128, 3)
        # 对于V通道：直接比较每个网格点
    ...
    if epoch % save_interval == 0:
        ...
        if test_loss < best_test_loss:
            ...
            best_ckpt_path = f"results_schrodinger_unet/model_checkpoint_best_{best_test_loss:.6f}.pt"
        ...
        plt.grid(True)
        plt.yscale('log')
        plt.savefig("results_schrodinger_unet/loss_curve.png")
        plt.close()
        # 可视化一个测试样本
        model.eval()
        ...
        with torch.no_grad():
            sample_pred = model(sample_input)  # (1, 128, 3)
        ...
        plt.tight_layout()
        plt.savefig(f"results_schrodinger_unet/{epoch}_pred_vs_true.png")
        plt.close()


np.save("results_schrodinger_unet/train_losses.npy", np.array(train_losses))
np.save("results_schrodinger_unet/test_losses.npy", np.array(test_loss_curve))
