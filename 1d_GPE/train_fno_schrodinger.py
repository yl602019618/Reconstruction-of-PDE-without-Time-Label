...
os.makedirs("results_schrodinger_fno", exist_ok=True)
...
for epoch in tqdm(range(1, num_epochs + 1)):
    ...
    if epoch % save_interval == 0:
        ...
        if test_loss < best_test_loss:
            ...
            best_ckpt_path = f"results_schrodinger_fno/model_checkpoint_best_{best_test_loss:.6f}.pt"
        ...
        plt.grid(True)
        plt.yscale('log')
        plt.savefig("results_schrodinger_fno/loss_curve.png")
        plt.close()
        # 可视化一个测试样本
        model.eval()
        ...
        plt.tight_layout()
        plt.savefig(f"results_schrodinger_fno/{epoch}_pred_vs_true.png")
        plt.close()


np.save("results_schrodinger_fno/train_losses.npy", np.array(train_losses))
np.save("results_schrodinger_fno/test_losses.npy", np.array(test_loss_curve))
