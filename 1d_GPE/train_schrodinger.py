...
from NIOModules import NIOFP, NIOFP_schrodinger
...
num_epochs = 1000
save_interval = 10
os.makedirs("results_schrodinger", exist_ok=True)
...
for epoch in tqdm(range(1, num_epochs + 1)):
    ...
    if epoch % save_interval == 0:
        ...
        plt.grid(True)
        plt.yscale('log')
        plt.savefig("results_schrodinger/loss_curve.png")
        plt.close()
        # 可视化一个测试样本
        model.eval()
        ...
        plt.tight_layout()
        plt.savefig(f"results_schrodinger/{epoch}_pred_vs_true.png")
        plt.close()
