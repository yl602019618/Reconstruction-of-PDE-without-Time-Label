...
class SpectralConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2):
        ...
        """
        2D Fourier layer without using cfloat.
        使用实数（最后一维大小为2）来表示复数，完成 FFT、线性变换以及逆 FFT。
        """
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.modes1 = modes1  # 选取的 Fourier 模式数
        self.modes2 = modes2

        self.scale = 1 / (in_channels * out_channels)
        # 定义参数，最后一维大小为2代表 [real, imag]
        self.weights1 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, modes1, modes2, 2, dtype=torch.float32))
        self.weights2 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, modes1, modes2, 2, dtype=torch.float32))

    def compl_mul2d(self, input, weights):
        """
        复数乘法，均以实数形式表示，最后一维表示 [real, imag]
        输入 shape: (batch, in_channel, x, y, 2)
        权重 shape: (in_channel, out_channel, x, y, 2)
        输出 shape: (batch, out_channel, x, y, 2)
        """
        # 计算实部： real = input.real * weights.real - input.imag * weights.imag
        real = torch.einsum("bixy,ioxy->boxy", input[..., 0], weights[..., 0]) - \
               torch.einsum("bixy,ioxy->boxy", input[..., 1], weights[..., 1])
        # 计算虚部： imag = input.real * weights.imag + input.imag * weights.real
    ...
    def forward(self, x):
        ...
        # 执行 2D FFT，并转换为实数形式 (最后一维大小为2，代表复数的 [real, imag])
        x_ft = torch.fft.rfft2(x)  # 输出 shape: (batch, in_channels, H, W//2+1) 的 cfloat
        x_ft = torch.view_as_real(x_ft)  # 转换为实数表示，shape: (batch, in_channels, H, W//2+1, 2)

        # 构造输出傅里叶张量，使用实数表示
        out_ft = torch.zeros(batchsize, self.out_channels, H, W // 2 + 1, 2, dtype=torch.float32, device=x.device)
        ...
        x_rec = torch.fft.irfft2(out_ft_complex, s=(H, W))
        return x_rec
