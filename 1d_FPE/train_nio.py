...
os.makedirs("results_nio", exist_ok=True)
...
for epoch in tqdm(range(1, num_epochs + 1)):
    ...
    if epoch % save_interval == 0:
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f"results_nio/loss_curve.png")
        plt.close()
        if test_loss < best_test_loss:
            best_test_loss = test_loss
            if best_ckpt_path is not None and os.path.exists(best_ckpt_path):
                os.remove(best_ckpt_path)
            best_ckpt_path = f"results_nio/model_checkpoint_best_{best_test_loss:.6f}.pt"
        ...
        plt.legend()
        plt.savefig(f"results_nio/potential_prediction_{epoch}.png")  # 保存 potential 图
        plt.close()
        # 绘制 drag 的对比图
        plt.figure()
        ...
        plt.ylim(-2,2)
        plt.legend()
        plt.savefig(f"results_nio/drag_prediction_{epoch}.png")  # 保存 drag 图
        plt.close()
        

np.save("results_nio/train_losses.npy", np.array(train_losses))
np.save("results_nio/test_losses.npy", np.array(test_losses))
np.save("results_nio/test_losses_drift.npy", np.array(test_losses_drift))
np.save("results_nio/test_losses_diffusion.npy", np.array(test_losses_diffusion))
